...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('submitBtn').addEventListener('click', function() {
        ...
        const roomTypes = document.querySelector('input[name="room-type"]:checked');
        let totalCost = 0;
    
        //extra meal cost for kids
        let mealCost=numChildren*5000;
        ...
        let total = (totalCost * numRooms * duration)+mealCost;
        ...
        // Display or use the 'total' variable as needed
        ...
        document.getElementById('overallCostOfBooking').innerText = `Current cost of booking: Rs. ${total.toFixed(2)}`;

        // Display overall booking
        //const selectedRoomTypes = Array.from(roomTypes).map(roomType => roomType.value).join(', ');
        document.getElementById('overallBooking').innerText = `Overall Booking:  ${roomTypes.value}`;
...
document.getElementById('addToFavourite').addEventListener('click', function() {
    const name = document.getElementById('name').value;
      ...
      const favourite= `Thank you ${name} for reserving ${numChildren} children & ${numAdults} adults for ${numRooms} days`;
      alert(favourite);
...
function favouritesBuuton(){
    ...
    const lastMessage = `Thank you for reserving. The total amount reserved is ${currentAdventureCost}.00 LKR`;
    
    let lastMessage_seraialized = JSON.stringify(lastMessage);
    localStorage.setItem("last message",lastMessage_seraialized);
}
